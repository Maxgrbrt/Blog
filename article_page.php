...
// Suppression d'un commentaire sur la page
if (isset($_GET['id_com']) AND $admin == TRUE) {
    $id_com = $_GET['id_com'];
    $id_article = $_GET['id_article'];
// Requete SQL pour supprimer le commentaire en question
    $supprimer_com = "DELETE "
            . "FROM commentaire  "
            . "WHERE id = $id_com AND id_commentaire = $id_article ";

    //echo $select;
    /* @var $bdd PDO */
    $sth_del = $bdd->prepare($supprimer_com);
    if ($sth_del->execute() == TRUE) {
        // Notification pour dire a l'utilisateur que l'article a bien été supprimé
...
<div class="container">
    ...
    foreach ($tab_com as $value) {
        ...
        <p class="alert alert-secondary">Posté par : <?php
            ...
            // Mise en place d'un systeme qui permet à l'admin de supprimer un commentaire
            if ($admin == TRUE) {
                ?>
                <br /> 
                <a href="article_page.php?action=Lire&id_article=<?php echo $id_article; ?>&id_com=<?= $value['id'] ?>" class="btn btn-danger">Supprimer</a>
...
<form action="article_page.php?action=Lire&id_article=<?php echo $id_article; ?>" method="post" enctype="multipart/form-data" id="form_article_page">
