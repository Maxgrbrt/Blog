...
if (isset($_GET['search'])) {
    ...
    // Requete SQL pour compter le nombre d'article totaux correspondant à la recherche
    ...
    $sth->execute();
    // recupere le nombre d'articles publie (nombre de fois dans la base de donnée ou notre publie est a 1 avec notre recherche) et le met dans $tab_result
    $tab_result = $sth->fetch(PDO::FETCH_ASSOC);
    // On met cette valeur dans la variable nb_total_article_publie
    $nb_total_article_publie = $tab_result['nb_total_article_publie'];
}
...
if (isset($_GET['search'])) {
    ...
    if (!empty($recherche)) {
        ...
        $sql = "SELECT "
                ...
                . "DATE_FORMAT(date, '%d/%m/%Y') as date_fr "
                . "FROM articles WHERE(titre LIKE :recherche OR texte LIKE :recherche) "
                . "AND publie = 1 ORDER BY id DESC LIMIT :index, :nb_articles_par_page;";
...
// Je mets dans la variable tab_articles le corps de mon article 
        if ($sth->execute() == TRUE) {
            ...
            $count = $sth->rowCount();
            $notification = "$nb_total_article_publie Article(s) trouvé(s)";
